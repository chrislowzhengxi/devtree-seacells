"""
python plot_scanpy_hist_with_two_thresholds.py \
  --csv /project/imoskowitz/xyang2/chrislowzhengxi/results/score_genes/Lateral_plate_mesoderm/Lateral_plate_mesoderm_SHH_scoregenes_percell.csv \
  --col SHH_scoregenes \
  --nozeros \
  --out /project/imoskowitz/xyang2/chrislowzhengxi/results/score_genes/histograms/Lateral_plate_mesoderm_SHH_scoregenes_hist_nozeros_with_two_thresholds.png \
  --title "Lateral_plate_mesoderm SHH score_genes (no-zeros)"
"""
import argparse
import numpy as np
import pandas as pd
...
def main():
    ...
    # We only need mid + high peaks now (thr1 is fixed at 0)
    ...
    # Force thr1 to 0 for convenience (Holly request)
    thr1 = 0.0

    # Keep thr2 as the dip between mid and high peaks
    thr2 = None
    ...
    # Optional shading to show 3 groups: (<0), (0..thr2), (>thr2)
    if thr2 is not None:
    ...
    if thr2 is not None:
        ...
        print("[THR2] NA (could not detect mid/high peaks)")
