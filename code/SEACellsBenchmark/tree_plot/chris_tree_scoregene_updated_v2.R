...
print(igraph::vertex_attr_names(g))
print(igraph::edge_attr_names(g))

# Debug 
vcount(g)
length(V(g)$meta_group)
head(V(g)$meta_group)
...
cat("meta_group length:", length(V(g)$meta_group), "\n")
cat("missing mappings:",
    sum(is.na(new_id_map[V(g)$meta_group])), "\n")

print(head(setdiff(V(g)$meta_group, nodes_filt$meta_group)))
