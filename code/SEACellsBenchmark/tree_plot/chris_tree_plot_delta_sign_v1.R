#############################################
## chris_tree_plot_delta_sign_v1.R
## Plot global HH tree from devtree_graph.rds
## Edge color shows SIGNED delta:
...
# 1b) Node palette (kept from your template)
...
# Transform makes most edges closer to grey, only strong deltas get saturated.
d <- replace_na(E(g)$delta, 0)
E(g)$dc <- sign(d) * sqrt(abs(d))
...
############################################################
##  A. TREE layout with SIGNED delta edges (labels)
############################################################

pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_tree_HH_overlay_deltaSign_v1.pdf"),
    width = 24, height = 12)
...
############################################################
##  B. TREE layout with SIGNED delta edges (unlabeled)
############################################################

pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_tree_HH_overlay_unlabeled_deltaSign_v1.pdf"),
    width = 24, height = 12)
...
############################################################
##  C. SUGIYAMA layout with SIGNED delta edges (labels)
############################################################

pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_sugiyama_HH_overlay_deltaSign_v1.pdf"),
    width = 24, height = 12)
...
############################################################
##  D. SUGIYAMA layout with SIGNED delta edges (unlabeled)
############################################################

pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_sugiyama_HH_overlay_unlabeled_deltaSign_v1.pdf"),
    width = 24, height = 12)
...
pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_system_skeleton_deltaSign_v1.pdf"),
    width = 20, height = 10)
