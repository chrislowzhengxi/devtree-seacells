...
# message("DEBUG: edge delta summary in plot script")
# print(summary(E(g)$abs_delta))
# print(summary(E(g)$delta))

# # edge_df <- as_data_frame(g, what = "edges")
# # print(head(edge_df[, c("system", "from", "to", "abs_delta", "delta")]))
# edge_df <- as_data_frame(g, what = "edges")
# # 'from' is the numeric vertex id; use it to index V(g)$system
# edge_df$system <- V(g)$system[edge_df$from]
# print(head(edge_df[, c("system", "from", "to", "abs_delta", "delta")]))
...
# if (!"sh" %in% names(vertex.attributes(g))) {
#   V(g)$sh <- 0  # placeholder so plotting works
# }
# # Node aesthetics
...
# # # 1) Load graph and filtered nodes
# # g <- readRDS(file.path(OUTPUT_CHRIS_TREE, "devtree_graph.rds"))
...
# # # Edge aesthetics
# # E(g)$ew <- rescale(replace_na(E(g)$abs_delta, 0), to = c(0.2, 2))
# # E(g)$ec <- abs(replace_na(E(g)$delta, 0))^0.5

# # # Node aesthetics
# # V(g)$nv <- replace_na(V(g)$sh, 0)^0.5

# # #############################
# # ## A. TREE, labeled
# # #############################
# # pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_tree_HH_overlay_scoregenes.pdf"),
# #     width = 24, height = 12)
...
# #                  end_cap = circle(2, "mm"),
# #                  show.legend = TRUE) +
# #   scale_edge_colour_gradientn(colours = pal_crest_r, name = "|delta|") +
# #   scale_edge_width(range = c(0.2, 2.0)) +
# #   geom_node_point(aes(colour = nv), size = 2) +
# #   scale_colour_gradientn(colours = pal_crest_r, name = "Node Hh (sh)") +
# #   geom_node_text(aes(label = new_id), size = 1.5,
# #                  vjust = -0.6, check_overlap = TRUE) +
# #   theme_void() +
...
# # ## B. TREE, unlabeled
# # #############################
# # pdf(file.path(OUTPUT_CHRIS_TREE, "devtree_tree_HH_overlay_unlabeled_scoregenes.pdf"),
# #     width = 24, height = 12)
