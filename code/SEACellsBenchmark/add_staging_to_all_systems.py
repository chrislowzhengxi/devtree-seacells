...
# -------------------------
def add_meta_and_staging_to_adata(adata, df_meta, staging_order):

    if "cell_id" not in adata.obs.columns:
        raise KeyError("adata.obs missing cell_id")
    ...
    for c in cols_to_add:
        if c not in meta_idx.columns:
            raise KeyError(f"Metadata missing column: {c}")
...
def main():
    ...
    parser.add_argument("--csv-meta", required=True)
    ...
    for system_tag in args.systems:
        ...
        if system_tag in ["Neurons", "Other_Brain_spinal_cord"]:
            df_sys = df_meta[df_meta["system"] == "Brain_spinal_cord"].copy()
        else:
            df_sys = df_meta[df_meta["system"] == system_tag].copy()
        # ----------------------------

        print(f"Metadata rows used: {len(df_sys):,}")
