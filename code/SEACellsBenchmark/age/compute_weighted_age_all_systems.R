...
age_all <- bind_rows(all_results) %>%
  select(system, celltype_new, total_cells, age_weighted) %>%
  arrange(system, age_weighted)

write_tsv(age_all, "age_score_by_celltype_by_system.tsv")

cat("DONE. Wrote age_score_by_celltype_by_system.tsv\n")
